---
layout: page
permalink: /extracurriculars/
title: drumming
nav: true
nav_order: 2
---

I started playing drums at the age of 8 and have been playing intermittently ever since, mostly as a hobby. I've played with several bands and occasionally recorded drum cover videos.

<div class="row mt-3">
  <div class="col-sm-6 col-md-4 mt-3">
    {% include video.liquid path="https://www.youtube.com/embed/vrtRl6gWNWA" class="img-fluid rounded z-depth-1" %}
    <div class="caption">Linkin Park - Numb</div>
  </div>
  <div class="col-sm-6 col-md-4 mt-3">
    {% include video.liquid path="https://www.youtube.com/embed/QDSvmyEFoJQ" class="img-fluid rounded z-depth-1" %}
    <div class="caption">Linkin Park - In the End</div>
  </div>
  <div class="col-sm-6 col-md-4 mt-3">
    {% include video.liquid path="https://www.youtube.com/embed/R5GD8QqwopY" class="img-fluid rounded z-depth-1" %}
    <div class="caption">紅蓮華 (LiSA) - Drum Cover</div>
  </div>
  <div class="col-sm-6 col-md-4 mt-3">
    {% include video.liquid path="https://www.youtube.com/embed/U9MqfV0KvLQ" class="img-fluid rounded z-depth-1" %}
    <div class="caption">Butter-Fly (和田光司) – Drum Cover</div>
  </div>
  <div class="col-sm-6 col-md-4 mt-3">
    {% include video.liquid path="https://www.youtube.com/embed/lXMlakYByXo" class="img-fluid rounded z-depth-1" %}
    <div class="caption">玫瑰少年 (五月天 Mayday) – Drum Cover</div>
  </div>
  <div class="col-sm-6 col-md-4 mt-3">
    {% include video.liquid path="https://www.youtube.com/embed/JxF-EMoV-n4" class="img-fluid rounded z-depth-1" %}
    <div class="caption">Avalanche (Bring Me The Horizon) – Drum Cover</div>
  </div>
</div>

<div class="d-flex justify-content-center mt-4">
  <div class="text-center">Some of my favorite drum covers.</div>
</div>
